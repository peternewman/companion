...
class CloudRegion {
			...
			while (this.socket) {
				for await (let event of this.socket.listener('authenticate')) {
					...
					if (this.socket.authState !== 'authenticated') {
						this.logger.warning(`Connection lost authentication, retrying`)
				...
				for await (let data of socket.subscribe(`companionProc:${this.cloud.state.uuid}:${name}`, {
					...
					try {
						...
						if (this.state.enabled) {
							...
							this.logger.debug(`rpc error: ${e.message}`)
						}
					}
	...
	setState(draftState) {
		...
		if (!newState.enabled && draftState.error) {
	...
	timerTick() {
		if (this.socket !== undefined) {
			;(async () => {
				if (this.state.enabled && this.state.loginRetry >= 0) {
				...
				try {
					if (this.state.enabled && this.socket.state == this.socket.OPEN) {
