...
class CloudController extends CoreBase {
	...
	constructor(registry) {
		...
		this.setState({ uuid: nodeMachineId.machineIdSync({ original: true }) })
	...
	getBanks(clone = false) {
		const buttonControls = Object.values(this.controls.getAllControls()).filter(
			(c) => c.type === 'button' && c.cloud !== true
		)
		return buttonControls.map((control) => {
			const controlInfo = ParseControlId(control.controlId)
			return { page: controlInfo.page, bank: controlInfo.bank, data: control.toJSON(false).style }
		})
	}
	...
	handleCloudRegionStateSet(client, region, newState) {
		...
		if (this.regionInstances[region] !== undefined) {
			this.regionInstances[region].setState({ ...newState })
		}
	}
		...
		if (connections) {
			for (let region in connections) {
				...
				if (this.regionInstances[region]) {
					this.regionInstances[region].setState({ enabled: connections[region] })
				}
			}
		}
	}
	 ...
	 * @access protected
	 */
	async updateBank(page, bank, render) {
		if (render.style?.cloud !== true) {
			const updateId = v4()
			for (let region in this.regionInstances) {
				if (this.regionInstances[region]?.socketTransmit) {
					this.regionInstances[region].socketTransmit('companion-banks:' + this.state.uuid, {
